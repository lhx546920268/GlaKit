...
class WrapLinearLayout : ViewGroup {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        var wrapLine = dequeueWrapLine(null, null)

        //根据计算出的宽度，计算出所需要的行数
        for (i in 0 until childCount) {
            if (wrapLine.lineWidth + getChildAt(i).measuredWidth + horizontalSpace > width) {
                wrapLine = dequeueWrapLine(wrapLine, getChildAt(i))
            } else {
    ...
    private fun dequeueWrapLine(wrapLine: WrapLine?, child: View?) : WrapLine{

        if(wrapLine != null && child != null){
            wrapLine.addView(child)
            _wrapLines.add(wrapLine)
        }else{
            _reusedLines.clear()
