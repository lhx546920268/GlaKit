...
class ObservableMultipartBody internal constructor(
    ...
    private val progressCallback: ValueCallback<Float>? = null
) : RequestBody() {
    private val contentType: MediaType = "$type; boundary=$boundary".toMediaType()
    private var contentLength = -1L
    private var totalDidSend = 0L
        ...
        if (countBytes) {
            ...
            val forwardingSink = object: ForwardingSink(sink!!) {
                override fun write(source: Buffer, byteCount: Long) {
                    totalDidSend += byteCount
                    progressCallback.also {
                        it(totalDidSend / length)
                    }
    ...
    class Builder @JvmOverloads constructor(
        ...
        fun build(): ObservableMultipartBody {
            check(parts.isNotEmpty()) { "Multipart body must have at least one part." }
            return ObservableMultipartBody(boundary, type, parts.toImmutableList(), progressCallback)
        }
    }
