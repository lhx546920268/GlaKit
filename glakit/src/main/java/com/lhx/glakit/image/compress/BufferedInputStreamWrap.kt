...
class BufferedInputStreamWrap(inputStream: InputStream?): FilterInputStream(inputStream) {
     ...
     */
    @Volatile
    private var buf: ByteArray? = ArrayPoolProvide.sharedProvider[64 * 1024]
    ...
    @Synchronized
    fun fixMarkLimit() {
        markLimit = buf?.size ?: 0
    }

    @Synchronized
    fun release() {
        if (buf != null) {
            ArrayPoolProvide.sharedProvider.put(buf)
            buf = null
        }
    }
    ...
    @Throws(IOException::class)
    private fun fillbuf(localIn: InputStream, localBuff: ByteArray?): Int {
        var localBuf = localBuff
        ...
        if (markPos == 0 && markLimit > localBuf!!.size && count == localBuf.size) {
            ...
            val newBuf: ByteArray = ArrayPoolProvide.sharedProvider[newLength]
            ...
            buf = newBuf
            localBuf = buf
            ArrayPoolProvide.sharedProvider.put(oldBuf)
        } else if (markPos > 0 && localBuf != null) {
        ...
        markPos = 0
        count = 0
        val byteRead = if (localBuf != null) localIn.read(localBuf, pos, localBuf.size - pos) else 0
    ...
    override fun mark(readLimit: Int) {
        ...
        markLimit = max(markLimit, readLimit)
        markPos = pos
    }
    ...
    @Synchronized
    @Throws(IOException::class)
    override fun read(buffer: ByteArray, offsetp: Int, byteCount: Int): Int {
        // Use local ref since buf may be invalidated by an unsynchronized close()
        var offset = offsetp
        var localBuf = buf ?: throw streamClosed()!!
        // Arrays.checkOffsetAndCount(buffer.length, offset, byteCount);
        if (byteCount == 0) {
            return 0
        }
        val localIn: InputStream = `in` ?: throw streamClosed()!!
        var required: Int
        if (pos < count) {
            // There are bytes available in the buffer.
            val copyLength = min(count - pos, byteCount)
        ...
        while (true) {
            ...
            if (markPos == -1 && required >= localBuf.size) {
                ...
                // localBuf may have been invalidated by fillbuf
                if (localBuf != buf) {
                    localBuf = buf ?: throw streamClosed()!!
                }
                read = min(count - pos, required)
